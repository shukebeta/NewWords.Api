    ...
    public async Task<ExplanationResult> GetMarkdownExplanationAsync(string inputText, string targetLanguage, LlmConfigurationService.AgentConfig agent)
    {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++)
        {
            foreach (var currentModel in agent.Models)
            {
                ...
                try
                {
                    // Construct the full API endpoint URL for chat completions
                    ...
                    var responseObj = JsonSerializer.Deserialize<ApiCompletionResponse>(apiResponseJson, JsonOptions.CaseInsensitive);

                    if (responseObj?.Choices != null && responseObj.Choices.Length > 0 && !string.IsNullOrWhiteSpace(responseObj.Choices[0].Message.Content))
                    {
                ...
                catch (FlurlHttpException flurlEx)
                {
                    ...
                    Console.WriteLine($"Attempt {attempt}/{maxRetries}: HTTP Error with model {currentModel} for agent {agent.ApiProvider}: {flurlEx.Message}. Status: {statusCode}");
                    ...
                    if (statusCode.HasValue && statusCode >= 400 && statusCode < 500)
                    {
                        return new ExplanationResult
                        {
                            ...
                            ErrorMessage = $"Agent {agent.ApiProvider} (Model: {currentModel}) failed with HTTP {statusCode}: {flurlEx.Message}",
                            ModelName = currentModel
                        };
                    }
...
Remember: Your entire response must be ONLY the JSON object, starting with `{{` and ending with `}}`." ;
    ...
    private async Task<string> _SendApiRequestAsync(object requestBody, string apiKey, string apiRequestUrl)
    {
        try
        {
            // Use Flurl's fluent API directly with the URL
            var response = await apiRequestUrl
        ...
        {
            // 获取原始响应内容（如果有）
            string errorContent = await ex.GetResponseStringAsync().ConfigureAwait(false);

            // 重新抛出包含状态码的异常
            throw new FlurlHttpException(ex.Call, $"Response status code does not indicate success: {ex.StatusCode} ({ex.Message}). Content: {errorContent}", ex);
        }
    }
    ...
    private WordExplanationResult _ParseStructuredExplanationResponse(string apiResponse, string inputText)
    {
        ...
        try
        {
            ...
            var responseObj = JsonSerializer.Deserialize<ApiCompletionResponse>(apiResponse, JsonOptions.CaseInsensitive);
